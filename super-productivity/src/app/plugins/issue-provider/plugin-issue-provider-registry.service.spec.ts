...
describe('PluginIssueProviderRegistryService', () => {
  ...
  describe('register', () => {
    ...
    it('should return the registered IssueProviderKey', () => {
      const definition = createMockDefinition();

      const key = registerProvider(
        service,
        'ret-plugin',
        definition,
        'Ret',
      ...
      expect(key).toBe('plugin:ret-plugin');
    });

    it('should throw on duplicate registrations and preserve first registration', () => {
      const definition = createMockDefinition();
  ...
  describe('unregister', () => {
    ...
    it('should remove only the specific key when registeredKey is passed', () => {
      const def = createMockDefinition();
      // Manually register two providers under one pluginId (custom keys)
      service.register({
        pluginId: 'multi',
        definition: def,
        name: 'A',
        ...
        icon: 'i',
        pollIntervalMs: 0,
        issueStrings: { singular: 'a', plural: 'as' },
        issueProviderKey: 'CUSTOM_A',
      });
      service.register({
        pluginId: 'multi',
        definition: def,
        ...
        icon: 'i',
        pollIntervalMs: 0,
        issueStrings: { singular: 'b', plural: 'bs' },
        issueProviderKey: 'CUSTOM_B',
      });

      expect(service.getRegisteredKeys('multi')).toEqual(
        jasmine.arrayContaining(['CUSTOM_A', 'CUSTOM_B']),
      );

      service.unregister('multi', 'CUSTOM_A');

      expect(service.hasProvider('CUSTOM_A')).toBeFalse();
      expect(service.hasProvider('CUSTOM_B')).toBeTrue();
      expect(service.getRegisteredKeys('multi')).toEqual(['CUSTOM_B']);
    });
  });

  describe('getRegisteredKeys', () => {
    it('should return all registered keys for a plugin', () => {
      const def = createMockDefinition();
      service.register({
        pluginId: 'multi-k',
        definition: def,
        name: 'K1',
        humanReadableName: 'K1',
        icon: 'i',
        pollIntervalMs: 0,
        issueStrings: { singular: 'a', plural: 'as' },
        issueProviderKey: 'KEY_1',
      });
      service.register({
        pluginId: 'multi-k',
        definition: def,
        name: 'K2',
        humanReadableName: 'K2',
        icon: 'i',
        pollIntervalMs: 0,
        issueStrings: { singular: 'b', plural: 'bs' },
        issueProviderKey: 'KEY_2',
      });

      expect(service.getRegisteredKeys('multi-k')).toEqual(
        jasmine.arrayContaining(['KEY_1', 'KEY_2']),
      );
    });

    it('should return an empty array for an unknown plugin', () => {
      expect(service.getRegisteredKeys('unknown')).toEqual([]);
    });
  });
