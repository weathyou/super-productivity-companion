...
export class PluginBridgeService implements OnDestroy {
    ...
    return {
      ...
      unregisterIssueProvider: () => {
        const registeredKeys =
          this._pluginIssueProviderRegistry.getRegisteredKeys(pluginId);
  ...
  private _registerIssueProvider(
    ...
    const registeredKey = this._pluginIssueProviderRegistry.register({
      pluginId,
      definition,
      name,
      humanReadableName,
  ...
  unregisterPluginHooks(pluginId: string): void {
    ...
    const registeredKeys = this._pluginIssueProviderRegistry.getRegisteredKeys(pluginId);
    this._pluginIssueProviderRegistry.unregister(pluginId);
    registeredKeys.forEach((registeredKey) =>
      this._syncAdapterRegistry.unregister(registeredKey),
    );
