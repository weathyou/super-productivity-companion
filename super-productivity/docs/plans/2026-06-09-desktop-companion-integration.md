...
- The launcher requires ports `4200` and `3876` to be free before starting. This prevents Super Productivity Electron from accidentally loading an Angular dev server from an external checkout, and prevents companion commands from hitting a non-isolated Super Productivity instance.
- Passing `-StopConflictingPorts` to the launcher explicitly stops processes listening on `4200` or `3876` before starting the isolated GUI session. The default behavior remains fail-fast only.
...
- Follow-up found that port `4200` was occupied by an Angular dev server from the external historical checkout `D:\AI_coding\super-productivity`; the isolated launcher was hardened so this now fails fast instead of launching against the wrong frontend.
- Running the launcher with `-StopConflictingPorts` stopped the external `4200` dev server and started a new isolated session. `GET http://localhost:4200` returned 200 and the listening process command line pointed to `D:\AI_coding\super-productivity-companion\super-productivity`. Clawd again wrote isolated runtime port `23334` and `GET /state` returned OK. Super Productivity `/health` remained unavailable until the desktop companion setting is enabled in the temporary Electron profile.

## 2026-06-10 GUI verification checkpoint

- Added a DEV-only verification flag, `SP_FORCE_DESKTOP_COMPANION=1`, for isolated GUI sessions. It starts the Super Productivity companion command server and allows renderer state publishing without changing the production default or requiring unsafe edits to the temporary Electron profile database.
- The isolated GUI launcher now sets `SP_FORCE_DESKTOP_COMPANION=1` only for the launched Super Productivity Electron process and waits for `http://localhost:4200` to become reachable before opening Electron. This prevents Electron from loading before the Angular dev server is ready.
- GUI session `C:\Users\haozhewang\AppData\Local\Temp\sp-companion-gui-20260610-135335` started the integrated Angular dev server, Super Productivity Electron, and Clawd from inside `D:\AI_coding\super-productivity-companion`.
- `GET http://127.0.0.1:3876/health` returned `{ ok: true, data: { server: "up", rendererReady: true } }`, confirming the real Electron renderer was connected to the local companion command server.
- `GET http://127.0.0.1:23334/state` returned `{ ok: true, app: "clawd-on-desk", port: 23334 }`, confirming Clawd's isolated runtime discovery.
