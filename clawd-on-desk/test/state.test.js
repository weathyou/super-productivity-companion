...
describe("productivity companion state", () => {
  let api;
  ...
  it("maps phase 1 productivity modes to visual states", () => {
    assert.strictEqual(api.updateProductivityState({
      source: "super-productivity",
      schemaVersion: 1,
      sentAt: Date.now(),
      state: {
        mode: "working",
        currentTask: { id: "task-1", title: "Demo" },
        timer: { isRunning: true },
      },
    }, { staleMs: 0 }), true);

    assert.strictEqual(api.getCurrentState(), "working");
    assert.strictEqual(api.getProductivityState().state.currentTask.id, "task-1");
  ...
  it("keeps productivity state separate from agent sessions and clears back to them", () => {
    ...
    mock.timers.tick(10000);
    assert.strictEqual(api.getCurrentState(), "idle");
    assert.ok(api.sessions.has("agent-1"));
    ...
    assert.strictEqual(api.getCurrentState(), "idle", "productivity source remains primary");

    assert.strictEqual(api.clearProductivityState(), true);
    mock.timers.tick(10000);
    assert.strictEqual(api.getProductivityState(), null);
    assert.strictEqual(api.getCurrentState(), "idle");
  });
});
