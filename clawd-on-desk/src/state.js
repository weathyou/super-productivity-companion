...
let productivitySnapshot = null;
let productivityStaleTimer = null;
const PRODUCTIVITY_STATE_STALE_MS = 15000;
...
function setState(newState, svgOverride, options = {}) {
  ...
  const productivityDisplayState = resolveProductivityDisplayState();
  if (
    productivityDisplayState
    && newState !== productivityDisplayState
    && typeof newState === "string"
    && !newState.startsWith("mini-")
  ) {
    newState = productivityDisplayState;
    svgOverride = getSvgOverride(productivityDisplayState);
  }
...
function mapProductivityModeToDisplayState(mode) {
  if (mode === "working") return "working";
  if (mode === "break") return STATE_SVGS.sleeping ? "sleeping" : "idle";
  if (mode === "attention" || mode === "overdue") return "notification";
  if (mode === "finishedDay") return "attention";
  return "idle";
}

function resolveProductivityDisplayState() {
  if (!productivitySnapshot || !productivitySnapshot.state) return null;
  return mapProductivityModeToDisplayState(productivitySnapshot.state.mode);
}

function clearProductivityState() {
  if (productivityStaleTimer) {
    clearTimeout(productivityStaleTimer);
    productivityStaleTimer = null;
  }
  if (!productivitySnapshot) return false;
  productivitySnapshot = null;
  applyResolvedDisplayState();
  return true;
}

function updateProductivityState(snapshot, options = {}) {
  if (!snapshot || !snapshot.state || typeof snapshot.state.mode !== "string") {
    return false;
  }
  productivitySnapshot = {
    ...snapshot,
    receivedAt: Number.isFinite(snapshot.receivedAt) ? snapshot.receivedAt : Date.now(),
  };
  if (productivityStaleTimer) {
    clearTimeout(productivityStaleTimer);
    productivityStaleTimer = null;
  ...
  const staleMs = Number.isFinite(options.staleMs)
    ...
    : PRODUCTIVITY_STATE_STALE_MS;
  if (staleMs > 0) {
    productivityStaleTimer = setTimeout(() => {
      productivityStaleTimer = null;
      if (!productivitySnapshot) return;
      productivitySnapshot = null;
      applyResolvedDisplayState();
    }, staleMs);
  }
  const displayState = resolveProductivityDisplayState();
  if (displayState) setState(displayState, getSvgOverride(displayState));
  return true;
}
...
function resolveDisplayState() {
  const productivityDisplayState = resolveProductivityDisplayState();
  if (productivityDisplayState) return productivityDisplayState;
...
function cleanup() {
  ...
  pendingState = null;
  if (autoReturnTimer) clearTimeout(autoReturnTimer);
  if (productivityStaleTimer) clearTimeout(productivityStaleTimer);
  productivityStaleTimer = null;
  productivitySnapshot = null;
  clearAllCompletionDebounces();
